          ...
          <div className="grid gap-10 lg:grid-cols-[1fr_1.2fr]">
            <SectionReveal>
              <div className="rounded-3xl border border-foreground/[0.08] bg-gradient-to-br from-terracotta/[0.08] via-sunset/[0.04] to-transparent p-10">
                <p className="text-[11px] font-semibold uppercase tracking-[0.28em] text-foreground/55">{t("visit")}</p>
                <h2 className="mt-4 font-serif text-3xl font-medium tracking-tight">
                  {t("findUsStart")} <span className="italic text-terracotta">{t("findUsAccent")}</span>
                </h2>
...
function ContactCard({
  ...
  return (
    ...
    >
      ...
      <ArrowUpRight
        size={18}
        className="absolute end-7 top-7 text-foreground/30 transition-all duration-500 group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-foreground"
      />
    </a>
  );
}
